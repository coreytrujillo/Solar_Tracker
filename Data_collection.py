	...
	while needtowrite == True:
		...
		# If file can't initiate
			...
			while needtooverwrite == True:
				a_response = input('There was an error initiating the file. Enter a to append, dd to delete and overwrite, or ee to exit\n')
...
def read_LJ_CA(handle):
	...
	aX = ljm.eReadName(handle, 'USER_RAM3_F32')
	aY = ljm.eReadName(handle, 'USER_RAM4_F32')
	aZ = ljm.eReadName(handle, 'USER_RAM5_F32')
	
	# Normalize magnetic values
	# mX = normalize_mag(-346.0, 563.0, mX0)
	# mY = normalize_mag(-525.0, 378.0, mY0)
	# mZ = normalize_mag(-557.0, 316.0, mZ0)
...
def check_PM(PM_fpath):
	...
	while lookPM:
		try:
			...
			numlines = len(PMf.readlines())
			PMf.close() # Get the number of lines
			if numlines >3:
				print('Power Meter file is long enough. Data should be collecting!')
				print(numlines)
				lookPM = False
			else:
				print('\n')
...
def read_PM(PM_fpath):
	PM_data = pd.read_csv(fpath, header = 2, index_col = 'Timestamp') # read power file
...
def build_header():
	...
	if ComAccelON == 1:
		header = header + ',Mag X,Mag Y,Mag Z,Accel X,Accel Y,Accel Z'
...
# This function reads all requested instruments outputs their values to outstring
...
def collect_data():
	...
	# Read Power Meter
	if PMON == 1:
		Power = read_PM(PM_floc, PM_fname)
